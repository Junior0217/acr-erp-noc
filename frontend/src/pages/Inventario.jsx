...
import {
  ...
  ArrowUpCircle, BarChart2, Loader2, Download, ShieldOff,
} from 'lucide-react'
...
function StockBadge({ stock }) {
  if (stock <= LOW_STOCK) return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-500/15 text-red-400 border border-red-500/30">
      <AlertTriangle size={10} /> {stock} — Stock Bajo
    </span>
  )
...
function TabProductos({ categorias, canCreate, canExport }) {
  ...
  async function eliminar(p) {
    ...
    if (r.status === 204) { toast.success('Producto eliminado.'); fetch_(page, debouncedSearch, catFiltro); return }
    const j = await r.json()
  ...
  function exportar() {
    ...
    exportCsv(`productos-${new Date().toISOString().slice(0, 10)}`, [
      ...
      { header: 'Stock',     getValue: r => r.stockActual },
    ], rows)
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row gap-3 p-4 border-b border-slate-800">
        ...
        {canExport && (
          <button onClick={exportar} className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-emerald-400 border border-slate-700 hover:border-emerald-600/40 transition-colors" title="Exportar vista actual a CSV">
            <Download size={14} /> <span className="hidden sm:inline">CSV</span>
          </button>
        )}
        {canCreate && (
          <button onClick={() => setModal('nuevo')} className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-lg transition-colors whitespace-nowrap">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px]">
          ...
          <tbody className="divide-y divide-slate-800">
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-600">Sin productos.</td></tr>
            )}
            {rows.map(p => (
      ...
      {modal && (
        <FormularioProducto
          ...
          onSaved={p => {
            setModal(null)
            fetch_(page, debouncedSearch, catFiltro)
...
function TabMovimientos({ canExport }) {
  ...
  function exportar() {
    ...
    exportCsv(`kardex-${new Date().toISOString().slice(0, 10)}`, [
      ...
      { header: 'Orden ID', getValue: r => r.orden?.id ?? '' },
      { header: 'Cliente',  getValue: r => r.orden?.servicio?.cliente?.razonSocial ?? '' },
    ], rows)
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row gap-3 p-4 border-b border-slate-800">
        ...
        {canExport && (
          <button onClick={exportar} className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-emerald-400 border border-slate-700 hover:border-emerald-600/40 transition-colors" title="Exportar Kardex a CSV">
            <Download size={14} /> <span className="hidden sm:inline">CSV</span>
          </button>
        )}
        ...
        <table className="w-full min-w-[640px]">
          <thead className="sticky top-0 z-10">
            <tr>
              ...
              <th className={TH}>Orden / Cliente</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800">
            {loading && rows.length === 0 && (
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Fibra · Equipos de Red · CCTV</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-700/50 rounded-xl overflow-hidden">
        ...
        {tab === 'productos'   && <TabProductos categorias={categorias} canCreate={canCreate} canExport={canExport} />}
